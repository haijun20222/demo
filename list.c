...
void reverse_node()
{
    struct node* p_first = phead;
    struct node* pre_node = phead->pnext;
    printf("**********reverse_node*****************\r\n");
    while(pre_node != NULL) 
    {
        p_first->pnext =  pre_node->pnext;//最开始节点的下一跳存储需要反转空间的指向的下一个空间
        pre_node->pnext = phead;          //反转的空间的下一跳指向前一个空间
        phead =  pre_node;                //head 指向反转的空间
        pre_node = p_first->pnext;        //pre_node 指向
    }
    p_first->data =  phead->data;
    phead->data = 0;
    traverse_node();
}
