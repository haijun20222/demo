...
ssize_t led_read(struct file * file, char __user *buff, size_t count, loff_t *loff);
ssize_t  led_write(struct file *file, const char __user *buff, size_t count, loff_t *loff);
...
#define C_BUFF_LEN  64
char c_buf[C_BUFF_LEN];
...
ssize_t led_read(struct file * file, char __user *buff, size_t count, loff_t *loff)
{
         ...
         ssize_t result;
         if(count>C_BUFF_LEN-1)
         {
               printk("helloworld 66\r\n");
		return -ENOMEM;
	  }
            
	  if(count<0)
	  {
	       printk("helloworld 71\r\n");
		return -EINVAL;
	  }
	  if(copy_to_user(c_buf,buff,count))
	  {
	      printk("helloworld 75\r\n");
	      printk("read %ld bytes %s\n",count,c_buf);
             return -EFAULT;
	  }
	  else
	  {
	       c_buf[63] = '\0';
	       printk("helloworld 80\r\n");
		printk("read %ld bytes %s\n",count,c_buf);
	  }
	  result = count;
	  return count;
...
ssize_t  led_write(struct file *file, const char __user *buff, size_t count, loff_t *loff)
{
	  ...
	  if(copy_from_user(c_buf,buff,count))
	  {
             return -EFAULT;
	  }
	  else
